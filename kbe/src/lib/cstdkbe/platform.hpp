...
typedef uint32													DBID;													// 一个在数据库中的索引用来当做某ID
...
inline std::wstring& kbe_replace(std::wstring& str, const wchar_t* old_value, const wchar_t* new_value)     
{
	size_t len = wcslen(old_value);
