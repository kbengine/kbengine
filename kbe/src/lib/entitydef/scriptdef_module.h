...
protected:
	...
	bool								hasCell_;			///< 是否有属性的Flags包含CELL
	bool								hasBase_;           ///< 是否有属性的Flags包含BASE
	bool								hasClient_;		    ///< 是否有属性的Flags包含CLIENT
    ...
    /// \b 如:
    /// Account, Avatar...
    /// 一般来讲def模块的文件即为entity_defs/\var name_.def
	std::string							name_;		
