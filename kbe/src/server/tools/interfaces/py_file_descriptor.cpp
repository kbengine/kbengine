...
{
	if(write)
		Interfaces::getSingleton().networkInterface().dispatcher().registerWriteFileDescriptor(fd_, this);
	else
		Interfaces::getSingleton().networkInterface().dispatcher().registerReadFileDescriptor(fd_, this);
}
...
{
	if(write_)
		Interfaces::getSingleton().networkInterface().dispatcher().deregisterWriteFileDescriptor(fd_);
	else
		Interfaces::getSingleton().networkInterface().dispatcher().deregisterReadFileDescriptor(fd_);
}
...
PyObject* PyFileDescriptor::__py_deregisterReadFileDescriptor(PyObject* self, PyObject* args)
{
	...
	PyFileDescriptor* pPyFileDescriptor = 
		static_cast<PyFileDescriptor*>(Interfaces::getSingleton().networkInterface().dispatcher().pPoller()->findForRead(fd));
...
PyObject* PyFileDescriptor::__py_deregisterWriteFileDescriptor(PyObject* self, PyObject* args)
{
	...
	PyFileDescriptor* pPyFileDescriptor = 
		static_cast<PyFileDescriptor*>(Interfaces::getSingleton().networkInterface().dispatcher().pPoller()->findForWrite(fd));
