...
{
	if(write)
		Loginapp::getSingleton().networkInterface().dispatcher().registerWriteFileDescriptor(fd_, this);
	else
		Loginapp::getSingleton().networkInterface().dispatcher().registerReadFileDescriptor(fd_, this);
}
...
{
	if(write_)
		Loginapp::getSingleton().networkInterface().dispatcher().deregisterWriteFileDescriptor(fd_);
	else
		Loginapp::getSingleton().networkInterface().dispatcher().deregisterReadFileDescriptor(fd_);
}
...
PyObject* PyFileDescriptor::__py_deregisterReadFileDescriptor(PyObject* self, PyObject* args)
{
	...
	PyFileDescriptor* pPyFileDescriptor = 
		static_cast<PyFileDescriptor*>(Loginapp::getSingleton().networkInterface().dispatcher().pPoller()->findForRead(fd));
...
PyObject* PyFileDescriptor::__py_deregisterWriteFileDescriptor(PyObject* self, PyObject* args)
{
	...
	PyFileDescriptor* pPyFileDescriptor = 
		static_cast<PyFileDescriptor*>(Loginapp::getSingleton().networkInterface().dispatcher().pPoller()->findForWrite(fd));
