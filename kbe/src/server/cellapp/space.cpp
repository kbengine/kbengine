...
PyObject* Space::__py_AddGeometryMapping(PyObject* self, PyObject* args)
{
	char* path = NULL;
	bool shouldLoadOnServer = true;
	PyObject* mapper = NULL;
	...
	if (!Space::addGeometryMapping(path, shouldLoadOnServer, params))
	{
...
bool Space::addGeometryMapping(std::string respath, bool shouldLoadOnServer, const std::map< int, std::string >& params)
{
