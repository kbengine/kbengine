...
{
	if(write)
		Dbmgr::getSingleton().networkInterface().dispatcher().registerWriteFileDescriptor(fd_, this);
	else
		Dbmgr::getSingleton().networkInterface().dispatcher().registerReadFileDescriptor(fd_, this);
}
...
{
	if(write_)
		Dbmgr::getSingleton().networkInterface().dispatcher().deregisterWriteFileDescriptor(fd_);
	else
		Dbmgr::getSingleton().networkInterface().dispatcher().deregisterReadFileDescriptor(fd_);
}
...
PyObject* PyFileDescriptor::__py_deregisterReadFileDescriptor(PyObject* self, PyObject* args)
{
	...
	PyFileDescriptor* pPyFileDescriptor = 
		static_cast<PyFileDescriptor*>(Dbmgr::getSingleton().networkInterface().dispatcher().pPoller()->findForRead(fd));
...
PyObject* PyFileDescriptor::__py_deregisterWriteFileDescriptor(PyObject* self, PyObject* args)
{
	...
	PyFileDescriptor* pPyFileDescriptor = 
		static_cast<PyFileDescriptor*>(Dbmgr::getSingleton().networkInterface().dispatcher().pPoller()->findForWrite(fd));
